...
const handleClickModifyAction = () => {
	...
	if (!addStatus.value) {
		...
		categoryApi.updateData(modifyCategoryInfo.value).then(result => {
			if (result.data === 1) {
				window.$message?.success(`修改类别 ${modifyCategoryInfo.value.title} 成功`)
		...
		categoryApi.insertData(modifyCategoryInfo.value).then(result => {
			if (!result.error) {
				window.$message?.success(`添加类别 ${modifyCategoryInfo.value.title}成功`)
				showDrawer.value = false
				emitter.emit(EnumMittEventName.reloadData)
			}
		})

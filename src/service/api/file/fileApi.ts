...
import {File} from "@/theme/pojo/file/file";
...
function _baseApi() {
	...
	function singleUploadFile(fileInfo: beanType, file: File, storageMode: number, userUid: number) {
		return request.post<viewBeanType>(apiPrefix + "singleUploadFile", { fileInfo, file, storageMode, userUid }, {
			headers: {
	...
	function multiUploadFile(files: File[], storageMode: number, userUid: number) {
		return request.post<Array<viewBeanType>>(apiPrefix + "multiUploadFile", { files, storageMode, userUid }, {
			headers: {
				'Content-Type': EnumContentType.formData
			}
		});
	...
	function typoraUploadFile(file: File, storageMode: number) {
		return request.post<string>(apiPrefix + "typoraUploadFile", { file, storageMode }, {
			headers: {
				'Content-Type': EnumContentType.formData
			}
		});
	...
	return { downloadFile, deleteFile, typoraUploadFile, multiUploadFile, singleUploadFile, queryOneData, queryOneDataByUid, queryListDataByCondition, batchUpdateData, updateData, logicDeleteData, physicalDeleteData, insertData, batchInsertData }
}
